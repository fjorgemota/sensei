...
class Sensei_Class_Student_Test extends WP_UnitTestCase {
	...
	public function testGetEnrolledCoursesQueryStaleIncluded() {
		...
		$this->assertEmpty( $user_meta, 'Student should not be calculated prior to querying courses' );
		...
		$user_meta = get_user_meta( $student_id, Sensei_Course_Enrolment_Manager::get_learner_calculated_version_meta_key(), true );
		$this->assertEquals( Sensei_Course_Enrolment_Manager::instance()->get_enrolment_calculation_version(), $user_meta, 'Student should have been calculated prior to querying courses' );
