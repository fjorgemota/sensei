...
if ( ! class_exists( 'Sensei_Email_Teacher_Quiz_Submitted', false ) ) :
	...
	class Sensei_Email_Teacher_Quiz_Submitted {
		...
		function trigger( $learner_id = 0, $quiz_id = 0 ) {
			...
			$this->subject = apply_filters( 'sensei_email_subject', sprintf( __( '[%1$s] Your student has submitted a quiz for grading', 'sensei-lms' ), get_bloginfo( 'name' ) ), $this->template );
			$this->heading = apply_filters( 'sensei_email_heading', __( 'Your student has submitted a quiz for grading', 'sensei-lms' ), $this->template );
